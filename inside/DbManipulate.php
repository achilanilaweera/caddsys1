...
if($data->num_rows>0){ 
   while( $row = $data->fetch_array(MYSQLI_ASSOC)){
	    ...
	    if($row['username']==$_SESSION['username']){
				  ...
				  $str.="<tr><tr id='".$row['admin_id']."'><td>".$row['name']." </td><td>".$row['email']."</td><td width='10px'>".$row['branch_id']."</td><td width='10px'>".$row['username']."</td><td>".$row['password']."</td><td><input type='button' class='ajaxedit w3-button w3-dark-grey w3-round-large w3-hover-blue-grey' value='Edit'/> <input type='button' class='ajaxdelete w3-button w3-red w3-round-large w3-hover-blue-grey' value='Delete'> <p style='color:red'> -You-</p></td></tr>";
				  
					}else{
						 $str.="<tr><tr id='".$row['admin_id']."'><td>".$row['name']."</td><td>".$row['email']."</td><td width='10px'>".$row['branch_id']."</td><td width='10px'>".$row['username']."</td><td>".$row['password']."</td><td><input type='button' class='ajaxedit w3-button w3-dark-grey w3-round-large w3-hover-blue-grey' value='Edit'/> <input type='button' class='ajaxdelete w3-button w3-red w3-round-large w3-hover-blue-grey' value='Delete'></td></tr>";
					}
...
function updateData($data,$con){
  ...
  $username= $con->real_escape_string($data['un']);
	...
	if(($pass==NULL)||($pass=='')){
		$sql = "update admin set name='$fname',email='$ename',branch_id='$branchid',username='$username' where username='$username'";
